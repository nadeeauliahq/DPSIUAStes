...
router.post('/make', authenticate, authorize(['pegawai', 'admin']), async (req, res, next) => {
    try {
        ...
        if (!productType || !startperiod || !endperiod) {
            return res.status(400).json({ message: 'productType, startperiod, dan endperiod diperlukan' });
        }
...
router.get('/view', authenticate, authorize(['manager', 'admin']), async (req, res, next) => {
    try {
        const { productType, startperiod, endperiod } = req.query;
